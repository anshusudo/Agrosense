...
function CropCopilotPanel({ farms = [] }) {
  ...
  return (
    <section className="crop-copilot-panel" style={{ ...panelStyle, marginTop: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 320px), 1fr))", gap: 16, marginTop: 14 }}>
        <form onSubmit={handleAnalyze} style={{ display: "grid", alignContent: "start", alignItems: "start", gap: 10, minWidth: 0 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text-color)" }}>Farm context</label>
          <select name="farmId" value={form.farmId} onChange={handleChange} style={{ width: "100%", boxSizing: "border-box", padding: 10, borderRadius: 8, border: "1px solid var(--input-border)", background: "var(--input-bg)", color: "var(--text-color)" }}>
          ...
          <input
            ...
            style={{ width: "100%", boxSizing: "border-box", padding: 10, borderRadius: 8, border: "1px solid var(--input-border)", background: "var(--input-bg)", color: "var(--text-color)" }}
          />
          ...
          <textarea
            ...
            style={{ width: "100%", boxSizing: "border-box", padding: 10, borderRadius: 8, border: "1px solid var(--input-border)", background: "var(--input-bg)", color: "var(--text-color)", resize: "vertical" }}
          />

          <label style={{ fontSize: 13, fontWeight: 600, color: "var(--text-color)" }}>Crop image</label>
          <input type="file" accept="image/*" onChange={handleImageChange} style={{ width: "100%", maxWidth: "100%", color: "var(--text-color)" }} />
        ...
        <div style={{ display: "grid", gap: 12, minWidth: 0 }}>
          ...
          <div style={{ border: "1px solid var(--border-color)", borderRadius: 12, padding: 16, background: "var(--card-bg)", boxShadow: "0 4px 6px var(--shadow-color)", color: "var(--text-color)" }}>
            ...
            {!result ? (
              ...
              <div style={{ display: "grid", gap: 14, minWidth: 0, overflowWrap: "anywhere", fontSize: 14, color: "var(--text-color)" }}>
                <div style={{ padding: "12px 14px", background: "var(--preview-bg)", borderRadius: "8px", borderLeft: "4px solid #1e88e5" }}>
                  <div style={{ marginBottom: 4, fontWeight: 700 }}>Summary</div>
                  <p style={{ margin: 0, lineHeight: 1.5 }}>{result.result?.summary}</p>
                </div>
                <div style={{ padding: "12px 14px", border: "1px solid var(--border-color)", borderRadius: "8px" }}>
                  <div style={{ marginBottom: 6, fontWeight: 700 }}>Image description</div>
                  <p style={{ margin: 0, lineHeight: 1.5 }}>{result.result?.imageDescription}</p>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 10 }}>
                  <div style={{ padding: "10px 12px", background: "var(--preview-bg)", borderRadius: "8px" }}>
                    <div style={{ fontWeight: 700, marginBottom: 4 }}>Likely issue</div>
                    <div style={{ color: "#d18b00", fontWeight: 600, lineHeight: 1.4 }}>{result.result?.likelyIssue}</div>
                  </div>
                  <div style={{ padding: "10px 12px", background: "var(--preview-bg)", borderRadius: "8px" }}>
                    <div style={{ fontWeight: 700, marginBottom: 4 }}>Crop health</div>
                    <div>{result.result?.cropHealth}</div>
                  </div>
                  <div style={{ padding: "10px 12px", background: "var(--preview-bg)", borderRadius: "8px" }}>
                    <div style={{ fontWeight: 700, marginBottom: 4 }}>Severity</div>
                    <div>{result.result?.severity}</div>
                  </div>
                  <div style={{ padding: "10px 12px", background: "var(--preview-bg)", borderRadius: "8px" }}>
                    <div style={{ fontWeight: 700, marginBottom: 4 }}>Confidence</div>
                    <div>{Math.round(Number(result.result?.confidence || 0) > 1 ? Number(result.result?.confidence) : Number(result.result?.confidence || 0) * 100)}%</div>
                  </div>
                </div>
                ...
                {Array.isArray(result.result?.preventionPlan) && result.result.preventionPlan.length > 0 ? (
                  <div style={{ padding: "12px 14px", border: "1px solid var(--border-color)", borderRadius: "8px" }}>
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>Prevention plan</div>
                    <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 1.5 }}>
