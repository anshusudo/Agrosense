...
function buildSystemPrompt({ cropType, notes, farmContext }) {
  return [
    ...
    'Return JSON only with the following keys: summary, imageDescription, likelyIssue, cropHealth, severity, confidence, immediateActions, preventionPlan, expertReview, notes.',
    ...
    generationConfig: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          cropHealth: { type: 'STRING' },
          severity: { type: 'STRING' },
          confidence: { type: 'NUMBER' },
          immediateActions: { type: 'ARRAY', items: { type: 'STRING' } },
          preventionPlan: { type: 'ARRAY', items: { type: 'STRING' } },
...
async function analyzeCropImageWithGroq({ imageBuffer, mimeType, cropType, notes, farmContext }) {
  if (!process.env.GROQ_VISION_MODEL) {
    throw new Error('No Groq vision model is configured for crop image analysis.');
  }
  ...
  const systemPrompt = [
    ...
    'Return ONLY valid JSON (no markdown, no code blocks) with these keys: summary, imageDescription, likelyIssue, cropHealth, severity, confidence, immediateActions, preventionPlan, expertReview, notes.',
  ...
  const userPrompt = 'Analyze this crop image and provide crop health advisory in JSON format.';
